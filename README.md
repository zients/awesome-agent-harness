...
- [World Monitor](https://github.com/koala73/worldmonitor) - Agent-native real-time global intelligence platform. A single root URL exposes RFC 8288 link discovery, an RFC 9727 api-catalog, OpenAPI 3.1, an MCP server card with OAuth 2.1, `llms.txt`, and 25 packaged agent skills covering country briefs, conflict and unrest events, maritime and military flight tracking, energy and supply-chain disruption, internet outages, cyber threats, market quotes, and prediction markets. Ships an MCP server, REST API, CLI, and Python/Ruby/Go SDKs alongside the dashboard and Tauri desktop app. AGPL-3.0 and self-hostable via docker-compose; public data sources work without keys, most feeds need free-tier signups, and Cloudflare Radar is paid. Note that the packaged skills target the hosted `api.worldmonitor.app` endpoints and require a WorldMonitor API key.
...
- [reverse-skill](https://github.com/zhaoxuya520/reverse-skill) - Skill router for reverse engineering, authorized penetration testing, and security research. Routes a task to the right methodology via 41 routing rules backed by a 163-case regression suite, spanning 85 `SKILL.md` modules for APK/Android, iOS, binary (IDA, radare2, Ghidra), .NET, frontend JS, malware and YARA, firmware/IoT, pwn and exploit development, API/GraphQL, supply chain, LLM security, and CTF workflows. Enforces a scope and authorization gate before acting on a target and records evidence-to-finding chains for reports. Client-neutral (Claude Code, Codex, Cursor, OpenCode). Requires Java, Node.js 22.12+, Python 3, and locally installed security tooling; several helper scripts are PowerShell-only.
